...
from app.core.user_repository import get_user_ai_preference
...
@router.post("/process-embeddings", response_model=EmbeddingProcessingResponse)
async def trigger_embedding_processing(background_tasks: BackgroundTasks, current_user: CurrentUser):
    from app.chat.embedding import EmbeddingService

    user_id = current_user["id"]

    if not get_user_ai_preference(user_id):
