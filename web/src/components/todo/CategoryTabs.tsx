...
function CategoryTab({
  ...
  return (
    ...
    >
      <span
        className="w-[8px] h-[8px] mr-1.5 rounded-full flex-shrink-0"
        style={{ backgroundColor: category.color }}
      />
      <span
        className={`text-[12px] font-normal text-black whitespace-nowrap overflow-hidden text-ellipsis ${isActive ? "font-semibold" : ""}`}
      >
        {category.name}
      </span>
    </div>
...
function ReorderableCategoryTab(
  props: CategoryTabProps & { value: string; onDragEnd?: () => void },
) {
...
export function CategoryTabs() {
  ...
  const categories: Category[] = useMemo(
    () => [
      ...
      ...customLists.map((list) => ({
        id: list.id,
        name: list.name,
        color: list.color || DEFAULT_CATEGORY_COLOR,
      })),
    ],
    [customLists],
  );
  ...
  const { handleReorder, handleReorderEnd } = useDeferredReorder(
    reorder,
    persistReorder,
  );
  ...
  return (
    <div className="flex flex-col w-full overflow-visible relative h-full items-center bg-transparent pt-0.5 whitespace-nowrap">
      <div
        className={`flex items-center w-full max-w-full relative bg-transparent ${isAddingCategory ? "pl-2 pr-1" : "px-3"} py-2 h-full gap-1 flex-nowrap justify-start overflow-visible`}
      >
        <div
        ...
        >
          <CategoryTab
            ...
            isActive={
              selectedListId === allCategory.id ||
              (!selectedListId && allCategory.id === "all")
            }
            ...
            >
              {categories.slice(1).map((category) => (
                <ReorderableCategoryTab
                  ...
                  category={category}
                  isActive={
                    selectedListId === category.id ||
                    (!selectedListId && category.id === "all")
                  }
                  onCategoryChange={handleCategoryChange}
                  onContextMenu={handleContextMenu}
        ...
        {isAddingCategory ? (
          <div
            className={`flex items-center gap-2 px-2 h-8 flex-1 min-w-0 w-full max-w-full rounded-md z-[15] animate-[slideIn_0.2s_ease-out] border border-[#e5e5ea] bg-white`}
          >
            <div
              className="relative flex items-center ml-[7px]"
              ref={colorPickerRef}
            >
              <button
                type="button"
