...
export function getEventColorPalette(event: CalendarEvent): EventColorPalette {
  ...
  if (event.colorId) {
    ...
    const matched = Object.entries(EVENT_COLORS).find(
      ([, value]) => value.googleId === event.colorId,
    );
    if (matched) {
      return matched[1];
    }
...
export function getEventDisplayStyles(
  ...
  else if (isPast || isCompleted) opacity = 0.7;
  ...
  const backgroundColor = showDashedBorder
