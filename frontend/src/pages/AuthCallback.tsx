...
import { fetchEncryptionKey } from '../lib/crypto'
...
export function AuthCallback() {
  ...
  const handleCallback = useCallback(async () => {
    ...
    const chronosState = searchParams.get('chronos_state')
    ...
    try {
      ...
      if (chronosState) {
        params.append('chronos_state', chronosState)
      }
      const response = await api.post<{ user: { id: string; email: string; name?: string; avatar_url?: string }; expires_at: number }>(`/auth/callback?${params.toString()}`)

      try {
        await fetchEncryptionKey(response.user.id)
      } catch {
        console.warn('Failed to fetch encryption key, will retry later')
      }
