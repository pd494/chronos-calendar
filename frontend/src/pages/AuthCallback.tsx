...
export function AuthCallback() {
  ...
  const handleCallback = useCallback(async () => {
    ...
    try {
      const response = await api.post<{ user: { id: string; email: string; name?: string; avatar_url?: string }; expires_at: number }>(
        `/auth/callback?code=${encodeURIComponent(code)}`
      )

      setAuthFromCallback(response.user, response.expires_at)
