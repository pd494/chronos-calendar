...
export function useCalendarSync({
  ...
  const processEvents = useCallback(async (payload: SSEEventsPayload) => {
    const dexieEvents: DexieEvent[] = payload.events.map((event) => ({
      ...calendarEventToDexie({
        ...
        attendees: event.attendees as DexieEvent['attendees'],
        organizer: event.organizer,
        reminders: event.reminders as DexieEvent['reminders'],
        conferenceData: event.conferenceData as DexieEvent['conferenceData'],
  ...
  const sync = useCallback(async () => {
    ...
    const syncPromise = new Promise<void>((resolve, reject) => {
      ...
      // Note: withCredentials is not supported in all browsers (e.g., some older mobile browsers).
      // In those cases, credentials will not be sent with the SSE request.
      ...
      eventSource.addEventListener('complete', async (e) => {
        try {
          const payload: SSECompletePayload = JSON.parse((e as MessageEvent).data)
          eventSource.close()
          eventSourceRef.current = null
