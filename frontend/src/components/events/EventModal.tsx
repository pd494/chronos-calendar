...
import { X, MapPin, Clock, Calendar, Users, Bell, Repeat, Check, Video, ChevronDown, ChevronUp } from 'lucide-react'
...
const COLOR_OPTIONS = Object.entries(EVENT_COLORS) as [EventColor, (typeof EVENT_COLORS)[EventColor]][]
...
function linkifyText(text: string): (string | { url: string })[] {
  ...
  return text.split(URL_REGEX).map((part) => {
    if (!part) return part
    if (part.match(/^https?:\/\//)) return { url: part.replace(/[.,;:!?)]+$/, '') }
    if (part.match(/^www\./)) return { url: `https://${part.replace(/[.,;:!?)]+$/, '')}` }
    return part
  })
}
...
function getReminderCount(reminders: { useDefault?: boolean; overrides?: { method: string; minutes: number }[] } | undefined): number {
  if (reminders?.useDefault || !reminders?.overrides?.length) return 0
  return reminders.overrides.length
}

const PARTICIPANT_COLORS = ['#1761C7', '#FF3B30', '#34C759', '#FF9500', '#AF52DE', '#FFD60A', '#00C7BE', '#FF2D55']

function getParticipantColor(email: string): string {
  const index = email.split('').reduce((acc, char) => acc + char.charCodeAt(0), 0) % PARTICIPANT_COLORS.length
...
function normalizeEventFormData(data: EventFormData) {
  return {
    ...
    reminders: {
      ...
      overrides: [...(data.reminders?.overrides ?? [])].sort((a, b) => a.minutes - b.minutes),
    },
...
export function EventModal() {
  ...
  const [customRecurrenceFreq, setCustomRecurrenceFreq] = useState<'DAILY' | 'WEEKLY' | 'MONTHLY' | 'YEARLY'>('WEEKLY')
  ...
  const [customReminderMethod, setCustomReminderMethod] = useState<ReminderMethod>('popup')
  const [customReminderValue, setCustomReminderValue] = useState('15')
  const [customReminderUnit, setCustomReminderUnit] = useState<'minutes' | 'hours' | 'days' | 'weeks' | 'on_date'>('minutes')
  const [customReminderRelation, setCustomReminderRelation] = useState<'before' | 'after'>('before')
  ...
  const [, setExpandedChips] = useState<Set<string>>(new Set())
  ...
  useEffect(() => {
    ...
    if (!!selectedEventId) {
      const isSwitching = prevSelectedEventId.current !== null && prevSelectedEventId.current !== selectedEventId
  ...
  useEffect(() => {
    if (isNew && activeEventId) {
      ...
      form.reset({
        ...
        reminders: existingEvent.reminders ?? { useDefault: false, overrides: [{ method: 'popup', minutes: 30 }] },
  ...
  useLayoutEffect(() => {
    ...
    if (isDescriptionExpanded) {
      ...
      descriptionOverlayRef.current && (descriptionOverlayRef.current.scrollTop = 0)
    }
  ...
  useEffect(() => {
    const handleClick = (e: Event) => {
      ...
      if (recurrenceRef.current?.contains(target) || recurrenceButtonRef.current?.contains(target)) return
      if (reminderRef.current?.contains(target) || reminderButtonRef.current?.contains(target)) return
      if (colorRef.current?.contains(target) || colorButtonRef.current?.contains(target)) return
  ...
  const toggleReminder = (minutes: number | null) => {
    ...
    const hasReminder = currentOverrides.some((override) => override.minutes === minutes)
    const nextOverrides: { method: 'email' | 'popup'; minutes: number }[] = hasReminder
  ...
  const saveCustomReminder = () => {
    ...
    if (customReminderUnit !== 'on_date') {
      ...
      const reminderDateObj = new Date(`${customReminderDate}T${customReminderTime}:00`)
    ...
    const currentOverrides = form.getValues('reminders')?.overrides ?? []
    const nextOverrides: { method: 'email' | 'popup'; minutes: number }[] = currentOverrides.filter(
      (override) => !(override.minutes === minutes && override.method === customReminderMethod)
    )
  ...
  }, [customReminderValue, customReminderUnit, customReminderRelation, customReminderDate, customReminderTime, customReminderMethod])
  ...
  useLayoutEffect(() => {
    ...
    const side = candidates.length > 0 ? candidates[0].side : (spaceRight >= spaceLeft ? 'right' : 'left')
  ...
  const currentSnapshot = JSON.stringify(normalizeEventFormData(currentValues as EventFormData))
  const defaultSnapshot = JSON.stringify(
    normalizeEventFormData((form.formState.defaultValues ?? getDefaultEventValues(undefined, defaultCalendarId)) as EventFormData)
  )
  ...
  const { ref: registerDescriptionRef, ...descriptionRegisterProps } = form.register('description')
    ...
    <>
      ...
      <div
        ...
        className={`fixed z-[4000] bg-white max-w-[calc(100vw-48px)] border border-gray-200 rounded-[22px] overflow-visible shadow-[0_25px_50px_-12px_rgba(0,0,0,0.25)] origin-center ${
          isTransitioning ? 'transition-[opacity,transform] duration-250 ease-out' : ''
      ...
      >
        {position && (position.connectorTop != null || position.connectorLeft != null) && (
          <>
        ...
        >
          ...
          <div
            ...
            style={{ paddingBottom: !form.watch('description') && !descriptionOverflowing ? 5 : undefined }}
          >
                    ...
                    <div className="relative">
                      <textarea
                        ...
                        style={{
                          ...
                          color: !isDescriptionFocused && form.watch('description') ? 'transparent' : undefined,
                          caretColor: !isDescriptionFocused && form.watch('description') ? 'transparent' : undefined
                        }}
                      />
                      {!isDescriptionFocused && form.watch('description') && (
                        <div
                          ...
                          style={{
                            lineHeight: `${DESCRIPTION_LINE_HEIGHT}px`,
                            height: isDescriptionExpanded ? 'auto' : descriptionOverflowing ? MAX_DESCRIPTION_PREVIEW_HEIGHT : 'auto',
                            maxHeight: isDescriptionExpanded ? 320 : undefined
                          }}
                          onClick={() => descriptionInputRef.current?.focus()}
                        >
                          {linkifyText(form.watch('description') ?? '').map((part, i) =>
                            typeof part === 'string' ? (
                      ...
                      if (e.key === 'Enter' || e.key === ',') {
                        ...
                        if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
                          ...
                          if (!current.some((a) => a.email.toLowerCase() === email)) {
                            form.setValue('attendees', [...current, { email, responseStatus: 'needsAction' }], { shouldDirty: true })
                            setParticipantEmail('')
                            setExpandedChips(new Set())
                          }
                        }
                      ...
                      {!participantsExpanded ? (
                        <div className="flex flex-col gap-1.5">
                          <div className="flex items-center justify-between gap-2">
                            {(() => {
                              ...
                              const awaiting = attendees.filter((a) => a.responseStatus !== 'accepted' && a.responseStatus !== 'declined').length
                              const parts = [going > 0 && `${going} going`, declined > 0 && `${declined} declined`, awaiting > 0 && `${awaiting} awaiting`].filter(Boolean)
                              return (
                                <div className="text-xs text-gray-500">
                                  <span className="font-medium text-gray-700">{attendees.length} guest{attendees.length !== 1 ? 's' : ''} –</span>
                            ...
                            <button
                              type="button"
                              onClick={(e) => { e.stopPropagation(); setParticipantsExpanded(true) }}
                          ...
                          {(form.watch('attendees') ?? []).slice(0, 5).map((attendee, i) => {
                            ...
                            return (
                              <div key={attendee.email} className="relative group" style={{ marginLeft: i > 0 ? -8 : 0, zIndex: 5 - i }}>
                                ...
                                <button
                                  type="button"
                                  onClick={(e) => { e.stopPropagation(); const list = form.getValues('attendees') ?? []; const idx = list.findIndex((a) => a.email === attendee.email); form.setValue('attendees', list.filter((_, j) => j !== idx), { shouldDirty: true }) }}
                                ...
                                >
                                  <X size={10} className="text-gray-600 hover:text-red-600" strokeWidth={2.5} />
                                </button>
                              </div>
                            )
                          })}
                          {(form.watch('attendees') ?? []).length > 5 && (
                            <div
                              ...
                              style={{ marginLeft: -5, zIndex: 0, width: 33.6, height: 33.6 }}
                            >
                              +{(form.watch('attendees') ?? []).length - 5}
                            </div>
                          )}
                        ...
                        <>
                          <div className="flex items-center justify-between gap-2 mb-1">
                            <div className="text-xs text-gray-500">
                              <span className="font-medium text-gray-700">{(form.watch('attendees') ?? []).length} guest{((form.watch('attendees') ?? []).length) !== 1 ? 's' : ''} –</span>
                              {(() => {
                                const attendees = form.watch('attendees') ?? []
                                const going = attendees.filter((a) => a.responseStatus === 'accepted').length
                                const declined = attendees.filter((a) => a.responseStatus === 'declined').length
                                const awaiting = attendees.filter((a) => a.responseStatus !== 'accepted' && a.responseStatus !== 'declined').length
                                const parts = [going > 0 && `${going} going`, declined > 0 && `${declined} declined`, awaiting > 0 && `${awaiting} awaiting`].filter(Boolean)
                                return parts.length > 0 ? <span className="ml-1">{parts.join(', ')}</span> : null
                              })()}
                            </div>
                            <button
                              type="button"
                              onClick={(e) => { e.stopPropagation(); setParticipantsExpanded(false) }}
                          ...
                          <div className="flex flex-col gap-0.5 custom-scrollbar overflow-y-auto max-h-[200px]">
                            {(form.watch('attendees') ?? []).map((attendee) => {
                              ...
                              return (
                                <div key={attendee.email} className="flex items-center gap-2 py-1.5 group min-h-[36px]">
                                  <div className="relative flex-shrink-0">
                                    <div
                                      className="rounded-full text-xs font-semibold text-white flex items-center justify-center"
                                      style={{ backgroundColor: getParticipantColor(attendee.email), width: 28, height: 28 }}
                                    >
                                      {getInitials(attendee.email)}
                                    </div>
                                    {isAccepted && (
                                  ...
                                  <div className="flex-1 min-w-0">
                                    <div className={`text-sm text-gray-900 truncate ${isOrganizer ? 'font-semibold' : ''}`}>{attendee.displayName || attendee.email}</div>
                                    {isOrganizer && <div className="text-xs text-gray-500">Organizer</div>}
                                  </div>
                                  <button
                                    type="button"
                                    onClick={() => { const list = form.getValues('attendees') ?? []; const idx = list.findIndex((a) => a.email === attendee.email); form.setValue('attendees', list.filter((_, j) => j !== idx), { shouldDirty: true }) }}
          ...
          <div className="px-4 py-2.5 border-b border-gray-100">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {(() => {
                ...
                const conf = existingEvent?.conferenceData as { hangoutLink?: string; entryPoints?: { entryPointType: string; uri: string }[] } | undefined
                const meetingLink =
                  conf?.hangoutLink ??
                  conf?.entryPoints?.find((ep) => ep.entryPointType === 'video' && ep.uri)?.uri ??
                  ''
                const isLocationUrl = !!location && (() => { try { return ['http:', 'https:'].includes(new URL(location).protocol) } catch { return false } })()
                const googleMapsLink = location && !isLocationUrl ? `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(location)}` : ''
          ...
          <div className="px-4 py-2.5 border-b border-gray-100">
            <div className="flex items-start gap-[9px]">
              <div className="flex flex-col items-center gap-[10px] flex-shrink-0" style={{ marginTop: 4 }}>
              ...
              <div className="flex-1" style={{ marginTop: 4 }}>
                {!isAllDayLocal && (
                  <div className="h-[20px] mb-[10px] flex items-center gap-2 text-sm text-gray-900">
                    ...
                    />
                    <span className="text-gray-400 font-semibold w-3 text-center">-</span>
                    <Controller
                      name="end"
                      control={form.control}
                      render={({ field }) => (
                    ...
                    <label className="ml-auto flex w-[180px] items-center justify-end cursor-pointer group">
                      ...
                      <span className="ml-3 text-xs leading-none text-gray-600">All day</span>
                    </label>
                  </div>
                )}
                {isAllDayLocal && (
                ...
                <div className="h-[20px] flex items-center gap-2 text-sm text-gray-900">
                  ...
                  >
                    ...
                    <span className="ml-3 whitespace-nowrap text-xs text-gray-600">{getRecurrenceLabel(form.watch('recurrence'))}</span>
                  </button>
                  {recurrenceOpen &&
                    ...
                    ReactDOM.createPortal(
                      <div
                        ...
                        style={{
                          bottom: window.innerHeight - recurrenceButtonRef.current.getBoundingClientRect().top + 8,
                          left: recurrenceButtonRef.current.getBoundingClientRect().right - 150,
                        }}
                      >
                        <div className="px-3 pb-2 text-[11px] font-medium uppercase tracking-[0.08em] text-gray-400">Recurrence</div>
                        ...
                        {RECURRENCE_OPTIONS.map((opt) => (
                          ...
                          >
                            <span className={`flex-1 ${getRecurrenceLabel(form.watch('recurrence')) === opt.label ? 'font-semibold' : 'font-medium'}`}>{opt.label}</span>
                            {getRecurrenceLabel(form.watch('recurrence')) === opt.label && <Check size={16} className="text-gray-400" />}
                          </button>
                        ))}
                        <button
                          type="button"
                          onClick={() => {
                            const currentRule = form.getValues('recurrence')?.[0] || ''
                            let freq = 'WEEKLY'
                            let interval = '1'
                            let byDay: string[] = []
                            ...
                            if (currentRule.startsWith('RRULE:')) {
                              ...
                              const intervalMatch = currentRule.match(/INTERVAL=(\d+)/)
                              if (intervalMatch) interval = intervalMatch[1]
                              
                              const byDayMatch = currentRule.match(/BYDAY=([^;]+)/)
                              if (byDayMatch) byDay = byDayMatch[1].split(',')
                            }
                            
                            setCustomRecurrenceFreq(freq as any)
                          ...
                          className="w-full flex items-center gap-2.5 px-3 py-3 text-sm text-gray-700 hover:bg-gray-50 transition-colors text-left"
                        >
                          <span className={`flex-1 ${getRecurrenceLabel(form.watch('recurrence')) === 'Custom' ? 'font-semibold' : 'font-medium'}`}>Custom...</span>
                          {getRecurrenceLabel(form.watch('recurrence')) === 'Custom' && <Check size={16} className="text-gray-400" />}
                        </button>
                      </div>,
                      document.body
                    )}
                  {customRecurrenceOpen &&
                    ...
                    ReactDOM.createPortal(
                      <div
                        ...
                        style={{
                          width: Math.max(280, recurrenceRef.current.getBoundingClientRect().width),
                          bottom: window.innerHeight - recurrenceRef.current.getBoundingClientRect().bottom,
                          left: window.innerWidth - recurrenceRef.current.getBoundingClientRect().right > 280 
                            ? recurrenceRef.current.getBoundingClientRect().left
                            : recurrenceRef.current.getBoundingClientRect().right - 280,
                        }}
                      >
                        <div className="flex flex-col gap-1">
                          <div className="flex items-center gap-2 mt-1 px-1">
                            <span className="text-[13px] text-gray-700 font-medium whitespace-nowrap w-20">Frequency:</span>
                            <div className="relative flex-1 px-2 py-1.5 hover:bg-gray-100 rounded-lg transition-colors focus-within:bg-gray-100 border border-transparent hover:border-gray-200 focus-within:border-gray-300">
                              <select
                                ...
                                onChange={(e) => setCustomRecurrenceFreq(e.target.value as any)}
                              ...
                              >
                                <option value="DAILY" className="font-medium text-gray-700">Daily</option>
                                <option value="WEEKLY" className="font-medium text-gray-700">Weekly</option>
                                <option value="MONTHLY" className="font-medium text-gray-700">Monthly</option>
                                <option value="YEARLY" className="font-medium text-gray-700">Yearly</option>
                              </select>
                              <div className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none flex flex-col justify-center z-0">
                                <svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <path d="M1 2.5L4 5.5L7 2.5" stroke="#6B7280" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                                </svg>
                              </div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-2 px-1 my-2">
                            <span className="text-[13px] text-gray-700 font-medium whitespace-nowrap">Every</span>
                            ...
                            <span className="text-[13px] text-gray-700 font-medium">
                              ...
                              {customRecurrenceFreq === 'MONTHLY' ? 'month' : ''}
                          ...
                          {customRecurrenceFreq === 'WEEKLY' && (
                            <div className="flex border border-gray-200 rounded-[6px] overflow-hidden mb-3 mx-1">
                              {['MO', 'TU', 'WE', 'TH', 'FR', 'SA', 'SU'].map((day, i) => {
                                const isSelected = customRecurrenceByDay.includes(day)
                                ...
                                return (
                                  <button
                                    ...
                                    onClick={() => {
                                      if (isSelected) {
                                        setCustomRecurrenceByDay(customRecurrenceByDay.filter(d => d !== day))
                                      } else {
                                        setCustomRecurrenceByDay([...customRecurrenceByDay, day])
                                      }
                                    }}
              ...
              {colorOpen &&
                ...
                ReactDOM.createPortal(
                  <div
                    ...
                    style={{
                      bottom: window.innerHeight - colorButtonRef.current.getBoundingClientRect().top + 8,
                    ...
                    <div className="custom-scrollbar max-h-80 overflow-y-auto py-1">
                      {COLOR_OPTIONS.map(([colorKey, colorValue]) => (
                        <button
                          ...
                          type="button"
                          onClick={() => {
                            form.setValue('color', colorKey, { shouldDirty: true })
                            setColorOpen(false)
                          }}
                          onMouseDown={(e) => e.stopPropagation()}
                          className="flex w-full items-center gap-2.5 px-3 py-2 text-left text-gray-900 transition-colors hover:bg-gray-50"
                        >
                          <span className="h-3.5 w-3.5 rounded-full flex-shrink-0" style={{ backgroundColor: colorValue.border }} />
                          <span className="flex-1 text-[13px] leading-none capitalize truncate">{colorKey}</span>
                          {watchedColor === colorKey && <Check size={14} className="text-gray-500 flex-shrink-0" />}
                        </button>
                      ))}
                    </div>
                  </div>,
              ...
              {reminderOpen &&
                ...
                ReactDOM.createPortal(
                  <div
                    ...
                    style={{
                      bottom: window.innerHeight - reminderButtonRef.current.getBoundingClientRect().top + 8,
                      left: reminderButtonRef.current.getBoundingClientRect().left,
                    }}
                  >
                    <div className="flex items-center justify-between gap-2 px-3 pb-2">
                      <span className="text-[11px] font-medium uppercase tracking-[0.08em] text-gray-400">Reminders</span>
                      <button
                        type="button"
                    ...
                    <div className="pr-1">
                      {REMINDER_OPTIONS.map((opt) => {
                        const selectedMinutes = form.watch('reminders')?.overrides?.map((override) => override.minutes) ?? []
                        const isSelected = opt.minutes === null ? selectedMinutes.length === 0 : selectedMinutes.includes(opt.minutes)
                        ...
                        >
                          <span className={`flex-1 ${isSelected ? 'font-semibold' : 'font-medium'}`}>{opt.label}</span>
                ...
                ReactDOM.createPortal(
                  <div
                    ...
                    style={{
                      ...
                      bottom: window.innerHeight - reminderRef.current.getBoundingClientRect().bottom,
                    ...
                    <div className="flex flex-col gap-1">
                      <div className="relative">
                        <select
                          ...
                          onChange={(e) => setCustomReminderMethod(e.target.value as ReminderMethod)}
                        ...
                        >
                          <option value="popup" className="font-medium text-gray-700">Push notification</option>
                          <option value="email" className="font-medium text-gray-700">Email</option>
                        </select>
                      ...
                      {customReminderUnit !== 'on_date' ? (
                        <div className="flex items-center gap-1 my-2">
                          ...
                          <div className="relative flex-[1.2] px-1 py-2 hover:bg-gray-100 rounded-lg transition-colors focus-within:bg-gray-100 border border-transparent hover:border-gray-200 focus-within:border-gray-300">
                            ...
                            >
                              <option value="minutes" className="font-medium text-gray-700">minutes</option>
                              <option value="hours" className="font-medium text-gray-700">hours</option>
                              <option value="days" className="font-medium text-gray-700">days</option>
                              <option value="weeks" className="font-medium text-gray-700">weeks</option>
                              <option value="on_date" className="font-medium text-gray-700">On date</option>
                            </select>
                            <div className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none flex flex-col justify-center z-0">
                              <svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M1 2.5L4 5.5L7 2.5" stroke="#6B7280" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                          ...
                          <div className="relative flex-1 px-1 py-2 hover:bg-gray-100 rounded-lg transition-colors focus-within:bg-gray-100 border border-transparent hover:border-gray-200 focus-within:border-gray-300">
                            <select
                              ...
                              onChange={(e) => setCustomReminderRelation(e.target.value as any)}
                              className="w-full bg-transparent border-none outline-none text-[13px] text-gray-700 cursor-pointer p-0 m-0 appearance-none text-center focus:ring-0 font-semibold pr-3 relative z-10"
                            >
                              <option value="before" className="font-medium text-gray-700">before</option>
                              <option value="after" className="font-medium text-gray-700">after</option>
                            </select>
                            <div className="absolute right-2 top-1/2 -translate-y-1/2 pointer-events-none flex flex-col justify-center z-0">
                              <svg width="8" height="8" viewBox="0 0 8 8" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M1 2.5L4 5.5L7 2.5" stroke="#6B7280" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
                        ...
                        <div className="flex flex-col gap-1 my-2">
                          <div className="flex items-center gap-2">
                            <span className="text-[13px] text-gray-500 w-6 px-1">On</span>
                            <div className="relative flex-1 px-2 py-1.5 hover:bg-gray-100 rounded-lg transition-colors focus-within:bg-gray-100 border border-transparent hover:border-gray-200 focus-within:border-gray-300">
                              <input
                                type="date"
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-[13px] text-gray-500 w-6 px-1">At</span>
                            <div className="relative flex-1 px-2 py-1.5 hover:bg-gray-100 rounded-lg transition-colors focus-within:bg-gray-100 border border-transparent hover:border-gray-200 focus-within:border-gray-300">
                              <input
                                type="time"
                ...
                <div className="relative">
                  <button
                    ...
                    className={`flex items-center gap-1.5 px-3 py-1.5 text-sm shrink-0 transition-colors ${
                      (() => {
                        const status = existingEvent?.attendees?.find((a) => a.self)?.responseStatus ?? 'needsAction'
                  ...
                  >
                    ...
                    <span className="whitespace-nowrap">
                      {(() => {
                        const status = existingEvent?.attendees?.find((a) => a.self)?.responseStatus ?? 'needsAction'
                        return status === 'accepted' ? 'Going' : status === 'declined' ? 'Not going' : 'Maybe'
                      })()}
                    </span>
                    <ChevronDown size={14} />
                  </button>
                  {rsvpOpen &&
                    ...
                    ReactDOM.createPortal(
                      <div
                        ...
                        style={{
                          width: rsvpButtonRef.current.getBoundingClientRect().width,
                          bottom: window.innerHeight - rsvpButtonRef.current.getBoundingClientRect().top + 8,
                          left: rsvpButtonRef.current.getBoundingClientRect().left,
                        }}
                      >
                        {RSVP_OPTIONS.map((option) => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 text-sm hover:bg-gray-50 ${
                              existingEvent?.attendees?.find((a) => a.self)?.responseStatus === option.value
                                ? 'font-semibold'
                                : ''
                            }`}
                          >
