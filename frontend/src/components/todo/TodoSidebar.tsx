...
export function TodoSidebar() {
  ...
  const handleToggleComplete = async (todo: Todo, isInCompletedList: boolean = false) => {
    if (isInCompletedList) {
      deleteTodo.mutate(todo.id)
    } else if (!todo.completed && completedList) {
      await toggleTodo.mutateAsync({ id: todo.id, completed: true })
      ...
      {user && (
        <div className="mt-auto py-3 px-3 border-t border-gray-200">
          <div className="relative" ref={userMenuRef}>
            {showUserMenu && (
              <div
                className="absolute left-0 bottom-full mb-3 bg-white border border-gray-200 rounded-2xl shadow-[0_10px_50px_rgba(0,0,0,0.15)] overflow-hidden z-[100] modal-fade-in w-full"
              >
