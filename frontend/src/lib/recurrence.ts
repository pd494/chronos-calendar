...
function computeCacheKey(
  ...
  const masterIds = masters.map((m) => `${m.id}:${m.updated}`).join(',')
  const exceptionIds = exceptions.map((e) => `${e.id}:${e.updated}`).join(',')
...
function parseRRuleString(rruleString: string, tzid?: string): RRuleSet | RRule | null {
  try {
    const result = rrulestr(rruleString, { forceset: true, tzid })
    return result
  } catch {
    return null
  }
...
function buildRRuleString(dtstart: Date, rruleStrings: string[], timeZone?: string): string {
  if (timeZone) {
    const localTime = formatDateTimeForTimezone(dtstart, timeZone)
    const formatted = localTime.replace(/[-:]/g, '')
    return `DTSTART;TZID=${timeZone}:${formatted}\n${rruleStrings.join('\n')}`
  }
...
export function expandRecurringEvents(
  ...
  for (const master of masters) {
    ...
    const fullRRule = buildRRuleString(dtstart, rruleStrings, isAllDay ? undefined : timeZone)
...
export function mergeEventsWithExpanded(
  ...
  return merged.sort((a, b) => {
    const aTime = a.start.dateTime ? new Date(a.start.dateTime).getTime() : new Date(a.start.date + 'T00:00:00').getTime()
    const bTime = b.start.dateTime ? new Date(b.start.dateTime).getTime() : new Date(b.start.date + 'T00:00:00').getTime()
    return aTime - bTime
  })
}
